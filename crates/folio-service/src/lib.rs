...
mod tests {
    ...
    use super::*;
    use folio_online::HttpResponse as OnlineHttpResponse;

    #[derive(Default)]
    struct QueueTransport {
        requests: Mutex<Vec<String>>,
        responses: Mutex<VecDeque<OnlineHttpResponse>>,
    }

    impl QueueTransport {
        fn with_responses(responses: Vec<OnlineHttpResponse>) -> Self {
            Self {
    ...
    impl HttpTransport for QueueTransport {
        fn get(&self, url: &str, limit: usize) -> Result<OnlineHttpResponse, OnlineError> {
            ...
            let response = self
                ...
                .unwrap()
                .pop_front()
                .ok_or_else(|| OnlineError::Unavailable("missing test response".to_owned()))?;
    ...
    fn online_json(body: &'static [u8]) -> OnlineHttpResponse {
        OnlineHttpResponse {
            status: 200,
    ...
    #[test]
    fn multipart_parser_preserves_binary_file_bytes_and_relative_name() {
        let boundary = "folioforge-test";
        ...
        body.extend_from_slice(
            format!(
                "--{boundary}\r\nContent-Disposition: form-data; name=\"files[]\"; filename=\"Series/Book.epub\"\r\nContent-Type: application/epub+zip\r\n\r\n"
            )
            .as_bytes(),
        );
        body.extend_from_slice(&[0, 1, 2, 0xff, 0, 3]);
        body.extend_from_slice(format!("\r\n--{boundary}--\r\n").as_bytes());
        let request = HttpRequest {
            method: "POST".to_owned(),
            path: "/convert".to_owned(),
        ...
        let form = parse_multipart(&request).unwrap();
        assert_eq!(form.field("target"), Some("kf7"));
        assert_eq!(form.files.len(), 1);
        assert_eq!(form.files[0].field, "files[]");
        assert_eq!(form.files[0].name, "Series/Book.epub");
        assert_eq!(form.files[0].bytes, [0, 1, 2, 0xff, 0, 3]);
    }

    #[test]
    fn phase_three_home_and_logo_routes_serve_the_branded_editor() {
        let state = AppState::new(ServiceConfig::default(), None);
        let home = route(
            &HttpRequest {
                method: "GET".to_owned(),
                path: "/".to_owned(),
        ...
        assert!(html.contains("Book Edit Plan"));
        assert!(html.contains("Compatibility Preview"));
        assert!(html.contains("document_titles"));
        assert!(html.contains("previewDevice"));
        assert!(html.contains("Optional online resources"));
        assert!(html.contains("Apply selected fields to Edit Plan"));
        assert!(html.contains("cover_asset_id"));

        let capabilities_response = route(
            &HttpRequest {
                method: "GET".to_owned(),
        ...
        assert_eq!(capabilities_response.status, "200 OK");
        let capabilities: serde_json::Value =
            serde_json::from_slice(&capabilities_response.body).unwrap();
        let formats = capabilities["input_formats"]
            .as_array()
            .unwrap()
            .iter()
            .filter_map(|item| item["format"].as_str())
            .collect::<Vec<_>>();
        assert!(formats.contains(&"Markdown"));
        assert!(formats.contains(&"HTML"));
        assert!(formats.contains(&"HTMLZ"));
        assert!(formats.contains(&"FB2"));
        assert!(formats.contains(&"DOCX"));
        let target_ids = capabilities["targets"]
            .as_array()
            .unwrap()
            .iter()
        ...
        let registry_ids = folio_core::FormatRegistry
            .output_formats()
            .into_iter()
            .map(|descriptor| descriptor.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(target_ids, registry_ids);
        assert!(target_ids.contains("KF7KF8Combo"));
        ...
        assert_eq!(logo.body, LOGO_BYTES);
    }

    #[test]
    fn online_providers_are_off_by_default_and_search_does_not_reach_network() {
        let state = AppState::new(ServiceConfig::default(), None);
        let status = route(
            &HttpRequest {
                method: "GET".to_owned(),
                path: "/online/status".to_owned(),
                headers: HashMap::new(),
                body: Vec::new(),
            },
            &state,
        );
        let status_value: serde_json::Value = serde_json::from_slice(&status.body).unwrap();
        assert_eq!(status_value["enabled"], false);
        assert_eq!(status_value["core_offline"], true);

        let search = route(
            &HttpRequest {
                method: "POST".to_owned(),
                path: "/online/metadata/search".to_owned(),
                headers: HashMap::new(),
                body: br#"{"query":"Pride and Prejudice"}"#.to_vec(),
            },
            &state,
        );
        assert_eq!(search.status, "503 Service Unavailable");
        assert!(String::from_utf8(search.body)
            .unwrap()
            .contains("disabled in offline mode"));
    }

    #[test]
    fn enabled_online_routes_search_download_and_cache_only_after_user_confirmation() {
        let transport = Arc::new(QueueTransport::with_responses(vec![
            online_json(
                br#"{"docs":[{"key":"/works/OL123W","title":"Example Book","author_name":["Example Author"],"cover_i":42}]}"#,
            ),
            online_json(
                br#"{"docs":[{"key":"/works/OL123W","title":"Example Book","author_name":["Example Author"],"cover_i":42}]}"#,
            ),
        ...
        let search = |path: &'static str| HttpRequest {
            method: "POST".to_owned(),
            path: path.to_owned(),
            headers: HashMap::new(),
            body: br#"{"query":"Example Book","title":"Example Book","author":"Example Author"}"#
                .to_vec(),
        };
        let metadata = route(&search("/online/metadata/search"), &state);
        assert_eq!(metadata.status, "200 OK");
        let metadata: serde_json::Value = serde_json::from_slice(&metadata.body).unwrap();
        assert_eq!(metadata[0]["metadata"]["title"], "Example Book");

        let covers = route(&search("/online/covers/search"), &state);
        assert_eq!(covers.status, "200 OK");
        let covers: serde_json::Value = serde_json::from_slice(&covers.body).unwrap();
        assert_eq!(covers[0]["provider"], "openlibrary_cover");
        assert!(covers[0]["requires_license_confirmation"]
            .as_bool()
            .unwrap());
        let download_body = |license_acknowledged| {
            serde_json::json!({
                "provider":"openlibrary_cover",
                "url":"https://covers.openlibrary.org/b/id/42-L.jpg?default=false",
                "cache_key":"cover:42",
        ...
        let download_request = |license_acknowledged| HttpRequest {
            method: "POST".to_owned(),
            path: "/online/download".to_owned(),
            headers: HashMap::new(),
            body: download_body(license_acknowledged),
        };
        let denied = route(&download_request(false), &state);
        assert_eq!(denied.status, "428 Precondition Required");
        assert_eq!(transport.request_count(), 2);

        let downloaded = route(&download_request(true), &state);
        assert_eq!(downloaded.status, "200 OK");
        assert_eq!(downloaded.content_type, "image/jpeg");
        assert_eq!(downloaded.body, [0xff, 0xd8, 0xff, 0xd9]);
        assert_eq!(
            downloaded
                .extra_headers
                .iter()
                .find(|(name, _)| *name == "x-folioforge-asset-id")
                .map(|(_, value)| value.as_str()),
            Some("openlibrary_cover%3Acover%3A42")
        );
        ...
        let form = MultipartForm {
            fields: HashMap::from([
                (
                    "cover_asset_id".to_owned(),
                    "openlibrary_cover:cover:42".to_owned(),
                ),
                ("cover_fit".to_owned(), "Fill".to_owned()),
            ]),
            files: Vec::new(),
        };
        let mut edit = BookEditPlan::default();
        apply_online_assets(&form, &state, &mut edit).unwrap();
        match edit.cover {
            CoverEdit::Replace {
                file_name,
                media_type,
                ...
                assert_eq!(file_name, "online-cover.jpg");
                assert_eq!(media_type, "image/jpeg");
                assert_eq!(bytes, [0xff, 0xd8, 0xff, 0xd9]);
                assert_eq!(fit, CoverFit::Fill);
            }
            _ => panic!("cached cover AssetId must create a CoverEdit"),
        }

        let cached = route(&download_request(true), &state);
        assert_eq!(cached.status, "200 OK");
        assert_eq!(transport.request_count(), 3);
    }

    #[test]
    fn online_metadata_endpoint_requires_confirmation_and_returns_a_book_edit_plan() {
        let request = |confirmed| HttpRequest {
            method: "POST".to_owned(),
            path: "/online/metadata/merge-plan".to_owned(),
            headers: HashMap::new(),
            body: serde_json::json!({
                "current": {"title":"Source title","authors":["Existing Author"]},
                "candidate": {
                    "candidate_id":"openlibrary:/works/OL123W",
                    "provider":"openlibrary",
                    "confidence":0.95,
                    "metadata":{"title":"Catalog title","authors":["New Author"]},
        ...
        let state = AppState::new(ServiceConfig::default(), None);
        let unconfirmed = route(&request(false), &state);
        assert_eq!(unconfirmed.status, "428 Precondition Required");

        let confirmed = route(&request(true), &state);
        assert_eq!(confirmed.status, "200 OK");
        let plan: serde_json::Value = serde_json::from_slice(&confirmed.body).unwrap();
        assert_eq!(plan["metadata"]["title"], "Catalog title");
        assert_eq!(
            plan["metadata"]["authors"],
            serde_json::json!(["Existing Author", "New Author"])
        );
    }

    #[test]
    fn multipart_edit_plan_accepts_metadata_and_replacement_cover() {
        let form = MultipartForm {
            fields: HashMap::from([
                (
                    "edit".to_owned(),
                    r#"{"metadata":{"title":"Prepared","clear_fields":[]},"cover":"Keep","typography":{},"fonts":{"strip_embedded_fonts":false},"styles":[],"structure":{"remove_navigation":false}}"#.to_owned(),
                ),
                ("cover_fit".to_owned(), "Fill".to_owned()),
            ]),
            files: vec![MultipartFile {
                field: "cover".to_owned(),
                name: "FolioForge.png".to_owned(),
                bytes: LOGO_BYTES.to_vec(),
            }],
        };
        let mut edit = edit_plan_from_form(&form).unwrap();
        apply_cover_upload(&form, &mut edit).unwrap();
        let mut source = folio_model::Book::new();
        source.metadata.title = Some("Original".to_owned());
        let edited = edit.apply(&source).unwrap();
        assert_eq!(source.metadata.title.as_deref(), Some("Original"));
        assert_eq!(edited.metadata.title.as_deref(), Some("Prepared"));
        assert_eq!(edited.resources.len(), 1);
        assert_eq!(edited.resources[0].media_type, "image/png");
        assert!(edited.resources[0]
            .properties
            .iter()
            .any(|value| value == "fit:fill"));
        assert_eq!(
            edited.load_resource(edited.resources[0].id, None).unwrap(),
            LOGO_BYTES
        );
    }

    #[test]
    fn multipart_edit_plan_accepts_a_signature_checked_replacement_font() {
        let form = MultipartForm {
            fields: HashMap::from([(
                "edit".to_owned(),
                r#"{"metadata":{"clear_fields":[]},"cover":"Keep","typography":{},"fonts":{"strip_embedded_fonts":false,"preferred_family":"Book Serif"},"styles":[],"structure":{"remove_navigation":false}}"#.to_owned(),
            )]),
            files: vec![MultipartFile {
                field: "font".to_owned(),
                name: "BookFont.ttf".to_owned(),
                bytes: vec![0, 1, 0, 0, 0],
            }],
        };
        let mut edit = edit_plan_from_form(&form).unwrap();
        apply_font_upload(&form, &mut edit).unwrap();
        let mut source = folio_model::Book::new();
        let style = source.styles.intern(folio_model::ComputedStyle::default());
        source.documents.push(folio_model::Document {
            id: folio_model::DocumentId::new(0),
            href: "book.xhtml".to_owned(),
            media_type: "application/xhtml+xml".to_owned(),
            title: None,
            nodes: vec![folio_model::Node::new(
                folio_model::NodeId::new(0),
                folio_model::NodeKind::Paragraph,
                style,
                Vec::new(),
            )],
        });
        ...
        assert_eq!(edited.resources.len(), 1);
        assert_eq!(edited.resources[0].kind, folio_model::ResourceKind::Font);
        assert_eq!(edited.resources[0].media_type, "font/ttf");
        assert_eq!(
            edited.load_resource(edited.resources[0].id, None).unwrap(),
            [0, 1, 0, 0, 0]
        );
        assert!(edited
            .styles
            .iter()
            .any(|(_, style)| style.get("font-family") == Some("Book Serif")));
    }

    #[test]
    fn multipart_replacement_font_rejects_invalid_magic_or_extension() {
        for (name, bytes) in [
            ("BookFont.ttf", b"not a font".to_vec()),
            ("BookFont.otf", vec![0, 1, 0, 0]),
        ] {
            let form = MultipartForm {
                fields: HashMap::new(),
                files: vec![MultipartFile {
            ...
            assert!(apply_font_upload(&form, &mut BookEditPlan::default()).is_err());
        }
    }

    #[test]
    fn service_accepts_only_the_three_public_degradation_modes() {
        assert_eq!(parse_mode("strict").unwrap(), DegradationMode::Strict);
        assert_eq!(
            parse_mode("compatible").unwrap(),
            DegradationMode::Compatible
        );
        assert_eq!(parse_mode("readable").unwrap(), DegradationMode::Readable);
        assert!(parse_mode("force").is_err());
    }

    #[test]
    fn preview_profile_is_validated_and_passed_as_a_target_setting() {
        let form = MultipartForm {
            fields: HashMap::from([
                ("preview_device".to_owned(), "phone".to_owned()),
                ("preview_orientation".to_owned(), "landscape".to_owned()),
                ("preview_font_size_percent".to_owned(), "145".to_owned()),
            ]),
            files: Vec::new(),
        };
        let settings = parse_preview_settings(&form).unwrap();
        assert_eq!(settings.device, folio_core::PreviewDevice::Phone);
        assert_eq!(
            settings.orientation,
            folio_core::PreviewOrientation::Landscape
        );
        assert_eq!(settings.font_size_percent, 145);

        let invalid = MultipartForm {
            fields: HashMap::from([("preview_font_size_percent".to_owned(), "500".to_owned())]),
            files: Vec::new(),
        };
        assert!(parse_preview_settings(&invalid).is_err());
    }

    #[test]
    fn upload_count_limit_rejects_excess_files() {
        let form = MultipartForm {
            fields: HashMap::new(),
            files: vec![
                MultipartFile {
        ...
        assert!(validate_file_count(&form, 1).is_err());
        assert!(validate_file_count(&form, 2).is_ok());
    }

    #[test]
    fn job_cancel_endpoint_signals_the_shared_cancellation_token() {
        let token = folio_core::CancellationToken::new();
        let mut state = AppState::new(ServiceConfig::default(), None);
        state.jobs = Arc::new(Mutex::new(HashMap::from([(
            "job-1".to_owned(),
            JobState {
                view: JobView {
                    id: "job-1".to_owned(),
        ...
        let response = route(
            &HttpRequest {
                method: "POST".to_owned(),
                path: "/jobs/job-1/cancel".to_owned(),
                headers: HashMap::new(),
                body: Vec::new(),
            },
            &state,
    ...
    fn test_job_state(id: &str, worker_done: bool, finished_at: Option<SystemTime>) -> JobState {
        JobState {
            view: JobView {
                id: id.to_owned(),
                status: if worker_done { "completed" } else { "running" }.to_owned(),
                progress: 1.0,
                message: "test job".to_owned(),
                report: None,
                error: None,
                download: None,
            },
            output_path: None,
            cancellation: folio_core::CancellationToken::new(),
            worker_done,
            finished_at,
        }
    }
    ...
    #[test]
    fn terminal_job_retention_is_bounded_and_never_removes_active_jobs() {
        let root = std::env::temp_dir().join(format!("folio-service-retention-{}", unique_id()));
        let work_dir = root.join("work");
        let jobs_dir = work_dir.join("jobs");
        fs::create_dir_all(&jobs_dir).unwrap();
        let now = SystemTime::now();
        let expired_id = "dead-beef".to_owned();
        let active_id = "face-cafe".to_owned();
        fs::create_dir_all(jobs_dir.join(&expired_id)).unwrap();
        fs::create_dir_all(jobs_dir.join(&active_id)).unwrap();
        ...
        jobs.insert(
            ...
            test_job_state(
                &expired_id,
                true,
                Some(now - JOB_RETENTION - Duration::from_secs(1)),
            ),
        );
        jobs.insert(active_id.clone(), test_job_state(&active_id, false, None));

        let mut oldest_retained_id = String::new();
        let mut newest_id = String::new();
        for index in 0..=MAX_RETAINED_JOBS {
            let id = format!("{index:x}-{index:x}");
            fs::create_dir_all(jobs_dir.join(&id)).unwrap();
            let age = Duration::from_secs(index as u64 + 1);
            jobs.insert(id.clone(), test_job_state(&id, true, Some(now - age)));
            if index == MAX_RETAINED_JOBS {
                oldest_retained_id = id.clone();
            }
            if index == 0 {
                newest_id = id;
            }
        }
        state.jobs = Arc::new(Mutex::new(jobs));

        prune_terminal_jobs(&state).unwrap();

        let jobs = state.jobs.lock().unwrap();
        assert_eq!(jobs.len(), MAX_RETAINED_JOBS + 1);
        assert!(jobs.contains_key(&active_id));
        assert!(jobs.contains_key(&newest_id));
        assert!(!jobs.contains_key(&expired_id));
        assert!(!jobs.contains_key(&oldest_retained_id));
        drop(jobs);
        assert!(jobs_dir.join(&active_id).is_dir());
        assert!(!jobs_dir.join(&expired_id).exists());
        assert!(!jobs_dir.join(&oldest_retained_id).exists());
        fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn active_job_permits_cap_concurrency_and_release_on_drop() {
        let active = Arc::new(AtomicUsize::new(0));
        let permits = (0..MAX_ACTIVE_JOBS)
            .map(|_| ActiveJobPermit::acquire(Arc::clone(&active)).unwrap())
            .collect::<Vec<_>>();
        assert_eq!(active.load(Ordering::Acquire), MAX_ACTIVE_JOBS);
        assert!(matches!(
            ActiveJobPermit::acquire(Arc::clone(&active)),
            Err(ServiceError::TooManyJobs)
        ));
        drop(permits);
    ...
    #[test]
    fn uncommitted_job_workspace_is_cleaned_by_its_guard() {
        let root = std::env::temp_dir().join(format!("folio-service-workspace-{}", unique_id()));
        fs::create_dir_all(&root).unwrap();
        fs::write(root.join("partial-upload"), b"temporary").unwrap();
        {
            let _guard = JobWorkspaceGuard::new(root.clone());
        }
        assert!(!root.exists());
    }

    #[test]
    fn upload_paths_cannot_escape_the_job_workspace() {
        assert_eq!(
            safe_relative_path("Series\\Book.epub").unwrap(),
            PathBuf::from("Series/Book.epub")
        );
        assert!(safe_relative_path("../Book.epub").is_err());
        assert!(safe_relative_path("/tmp/Book.epub").is_err());
    }
}
