...
mod tests {
    use super::*;
    use folio_model::{ComputedStyle, Document, DocumentId, Node, NodeId, NodeKind};
    ...
    #[test]
    fn plan_is_non_destructive_and_updates_metadata() {
        let source = book();
        let plan = BookEditPlan {
        ...
        let edited = plan.apply(&source).unwrap();
        assert_eq!(source.metadata.title, None);
        assert_eq!(edited.metadata.title.as_deref(), Some("Edited"));
    }

    #[test]
    fn css_is_interned_as_computed_style() {
        let source = book();
        let plan = BookEditPlan {
            styles: vec![StyleEdit {
                css: Some("color: #333; font-weight: bold".to_owned()),
                ..StyleEdit::default()
            }],
            ..BookEditPlan::default()
        };
        ...
        assert_eq!(
            edited
                .styles
                .get(edited.documents[0].nodes[0].style)
    ...
    #[test]
    fn font_replacement_is_signature_checked_and_embedded_in_the_prepared_ir() {
        let source = book();
        let plan = BookEditPlan {
            fonts: FontEdit {
                replacement: Some(FontReplacement {
                    file_name: "BookFont.ttf".to_owned(),
                    media_type: "font/ttf".to_owned(),
                    bytes: vec![0, 1, 0, 0],
                    family: Some("Book Serif".to_owned()),
                }),
                ..FontEdit::default()
            },
            ..BookEditPlan::default()
        ...
        assert_eq!(edited.resources[0].kind, ResourceKind::Font);
        assert_eq!(edited.resources[0].media_type, "font/ttf");
        assert_eq!(edited.font_faces.len(), 1);
        assert_eq!(edited.font_faces[0].family, "Book Serif");
        assert_eq!(
        ...
        let style = edited
            ...
            .get(edited.documents[0].nodes[0].style)
            .unwrap();
        assert_eq!(style.get("font-family"), Some("Book Serif"));
    }

    #[test]
    fn font_replacement_rejects_a_mismatched_signature() {
        let source = book();
        let plan = BookEditPlan {
            fonts: FontEdit {
                replacement: Some(FontReplacement {
                    file_name: "not-a-font.ttf".to_owned(),
                    media_type: "font/ttf".to_owned(),
